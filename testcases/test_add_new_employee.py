...
class Test_Add_Customer:
    ...
    def test_user_login(self, setup):
        ...
        self.logger.info("***************** Adding  new user details *****************")
        username = loginPageObj.check_customer_added()
        if username == "pradeep":
            self.logger.info("***************** User successfully added *****************")
            assert True
        else:
            self.logger.info("***************** User not added *****************")
            assert False
        self.logger.info("***************** logout from old user *****************")
        loginPageObj.logout()

        self.logger.info("***************** Login new User *****************")

        newloginPageObj = login(self.driver)
        self.logger.info("***************** new user is successfully login *****************")
